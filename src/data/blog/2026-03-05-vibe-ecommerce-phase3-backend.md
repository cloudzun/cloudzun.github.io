...
description: '从原型到产品：vibe-ecommerce 迭代系列（二） 引入后端，第一跳 这是 vibe-ecommerce 迭代系列的第二篇。
'
pubDatetime: 2026-03-05T00:00:00Z
tags: ['opencode', 'vibe-coding', '后端', 'node.js', 'sqlite']
---
...
上一篇
记录了 Phase 2 的前端打磨——搜索、排序、商品详情跳转、订单确认页。Phase 2 结束时，这个应用已经"看起来像"一个真实的电商网站。
但它不是。
所有商品数据硬编码在
js/data.js
，购物车存在 localStorage，订单数据存在 sessionStorage——关掉浏览器就消失。没有服务器，没有数据库，没有任何服务端状态。
Phase 3 要做的，是
第一跳
：从纯前端静态应用，跳到有真实后端的全栈应用。
一、为什么这一跳最难
...
这意味着：
部署变复杂
：前端在 Vercel，后端在 Azure Linux 服务器，需要反向代理、SSL、CORS
调试变困难
：问题可能出在前端、网络、代理、后端、数据库任何一层
架构决策变多
：用什么数据库？用什么 ORM？怎么管理进程？怎么处理跨域？
...
但有一个前提
：必须用 Knex.js 作为 Query Builder，而不是裸 SQL。
为什么 Knex 是关键决策
...
业务代码（routes/products.js、routes/orders.js）
一行不改
。
...
docker
exec
npm curl -s http://172.17.0.1:3001/health
# → failed
# 检查宿主机防火墙
...
经验
：当服务运行在 Docker 反代后面时，调试顺序应该是：
先从容器内部 curl 宿主机端口
再检查 iptables INPUT 规则
最后才看代理配置
...
失败红线
：sessionStorage 的订单数据必须被替换（不是补充）
...
无限流
server/app.js
Phase 5：express-rate-limit
iptables 规则持久化方式不规范
/etc/network/if-up.d/
Phase 5/6：迁移到 ufw
记录技术债不是为了让自己难受，是为了让未来的自己（或者接手的人）知道：
这里是有意为之的妥协，不是遗漏
。
七、思考与收获
收获 1：分阶段决策的价值
Phase 3 用 SQLite，Phase 5/6 迁移 Azure SQL DB。这不是"先凑合再说"，而是
把决策推迟到信息最充分的时候
。
...
API
：https://shop-api.huaqloud.com/api/products
GitHub
