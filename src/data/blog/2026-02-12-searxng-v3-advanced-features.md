...
description: 'SearxNG Wrapper v3.0：高级排序、过滤和多语言支持 📋 概述 继 v2.0 的 JSON API 和缓存层优化后，SearxNG Wrapper v3.0 引入了三项重要功能：高级排序、高级过滤和多语言支持。这些功能使搜索结果更加精准、相关，并支持全球化应用。
'
...
📋 概述
继 v2.0 的 JSON API 和缓存层优化后，SearxNG Wrapper v3.0 引入了三项重要功能：
高级排序
、
高级过滤
和
多语言支持
。这些功能使搜索结果更加精准、相关，并支持全球化应用。
发布日期：
2026-02-12
版本：
3.0-enhanced
...
1
curl
"http://127.0.0.1:8765/search?q=python+async&sort=relevance&api_key=KEY"
日期排序 (Date Sorting)
原理：
自动提取结果中的日期信息，按时间排序
支持格式：
...
# 只显示最近 7 天的结果
curl
"http://127.0.0.1:8765/search?q=news&days=7&api_key=KEY"
去重 (Deduplication)
1
2
3
...
# 自动检测所有结果的语言
curl
"http://127.0.0.1:8765/search?q=test&api_key=KEY"
...
api_key=KEY"
结果：
只显示官方 Python 网站、Stack Overflow 和 GitHub 上关于 Python 异步编程的最相关结果
场景 2：查找最新的新闻和文章
需求：
查找最近一周内关于 AI 的最新新闻
1
...
5
curl
"http://127.0.0.1:8765/search?q=artificial+intelligence+news&\
sort=date&\
days=7&\
remove_duplicates=true&\
api_key=KEY"
结果：
显示最近 7 天内关于 AI 的最新文章，按日期排序，去重
场景 3：多语言搜索
需求：
查找关于机器学习的英文、中文和日文资源
1
...
api_key=KEY"
结果：
显示英文、中文和日文的机器学习相关内容，按相关性排序
场景 4：排除特定网站
需求：
搜索 Python 相关内容，但排除社交媒体和购物网站
1
2
3
4
...
api_key=KEY"
结果：
显示 Python 相关内容，但排除社交媒体和购物网站，并移除垃圾内容
场景 5：学术研究
需求：
查找关于深度学习的最新学术资源
1
...
5
curl
"http://127.0.0.1:8765/search?q=deep+learning&\
sort=date&\
domains=arxiv.org,scholar.google.com&\
days=30&\
api_key=KEY"
结果：
显示最近 30 天内 arXiv 和 Google Scholar 上关于深度学习的最新学术论文
📈 性能数据
处理时间
操作
时间
