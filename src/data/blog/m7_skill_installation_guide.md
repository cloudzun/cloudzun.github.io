...
```bash
# 在 OpenClaw 中直接调用
sessions_spawn task="Run M7 stock analysis" --agentId main
```
...
# 查看特定日期的日志
grep "2026-02-13" /tmp/m7_analysis.log
...
```bash
# 检查 Python 依赖
pip list | grep -E "yfinance|requests"
...
# 检查 Hugo 博客目录
ls -l /home/chengzh/myblog/content/posts/ | tail -3
ls -l /home/chengzh/clean-vercel-blog/content/posts/ | tail -3
...
1. 阅读本文档的"快速开始"部分
2. 运行 `./m7_analysis.sh` 进行第一次分析
3. 查看生成的报告
4. 阅读 README.md 了解更多

### 中级用户

1. 学习使用 `--date` 和 `--skip-publish` 选项
2. 设置 Cron 定时运行
3. 查看日志和调试
4. 阅读 INVESTMENT_FRAMEWORK.md 理解分析方法

### 高级用户

1. 修改 Python 脚本以自定义分析
2. 集成到其他工作流
3. 扩展报告模板
4. 贡献改进和新功能
...
# 清理旧报告（可选）
find /tmp -name "m7_*.md" -mtime +30 -delete
```
...
```bash
# 分析多个日期
for date in 2026-02-13 2026-02-20 2026-02-27; do
  ./m7_analysis.sh --date $date
  sleep 60  # 等待 60 秒
done
```
...
```bash
# 后台运行分析
nohup ./m7_analysis.sh > /tmp/m7_analysis_$(date +%Y%m%d).log 2>&1 &

# 查看进程
ps aux | grep m7_analysis
```
...
```bash
# 分析完成后发送通知
./m7_analysis.sh && echo "M7 分析完成！" | mail -s "M7 Analysis Done" your@email.com
```
...
- 🐛 查看日志文件 `/tmp/m7_analysis.log` 调试问题
