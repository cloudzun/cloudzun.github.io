...
description: 'SearxNG 生产部署指南：从 Docker 容器到 systemd service + JSON API 本文档记录了在本地构建自托管 SearxNG 搜索引擎、并通过 Python wrapper 提供 JSON/RSS API 的完整过程，涵盖架构设计、配置要点、测试验证与运维强化。
'
pubDatetime: 2026-02-09T00:00:00Z
tags: ['searxng', 'search', 'docker', 'privacy', 'devops']
---
...
为什么选择 SearxNG？
隐私优先
：不记录用户查询，支持代理，可完全本地部署
多引擎聚合
：同时查询 DuckDuckGo、Brave、Qwant 等，综合排名更好
可定制
：支持自定义引擎清单、语言、分类、图片代理等
开源
：完全透明，可审计代码与配置
部署架构
┌─────────────────────────────────┐
...
1
2
curl -sS http://127.0.0.1:8080/search?q
=
test
|
head -c
...
步骤 3：部署 Wrapper
wrapper.py 的核心逻辑：
接收查询请求（GET/POST），验证 API key
转发给 SearxNG 的 HTML 响应
根据 format 参数：
...
# JSON 查询
curl -sS
"http://127.0.0.1:8765/search?q=python&format=json&api_key=
$API_KEY
"
|
jq
...
# RSS 查询
curl -sS
"http://127.0.0.1:8765/search?q=kubernetes&format=rss&api_key=
$API_KEY
"
|
grep
...
# 分类搜索
curl -sS
"http://127.0.0.1:8765/search?q=cat&categories=images&format=json&api_key=
$API_KEY
"
|
jq
...
# 多语言
curl -sS
"http://127.0.0.1:8765/search?q=test&language=de&format=json&api_key=
$API_KEY
"
|
jq
...
✅
分类搜索（10 类）
：全部通过（general=30, images=8009, videos=146, 等）
✅
格式支持
：HTML/JSON/RSS 均有效
✅
...
✅
多语言
：en, de, fr, zh-CN, auto 全部返回 200
✅
性能
：5 次顺序查询平均 1.12s（min=0.56s, max=1.62s）
✅
错误处理
：无 API key 被拒绝，空查询被拒绝
✅
认证
：API key 验证有效
详见
/home/chengzh/searxng/reports/comprehensive_test_20260209T094244Z.txt
生产化与强化
1. Docker 容器自动重启
已配置
restart: always
...
searxng-wrapper.service
systemd unit 路径：
/etc/systemd/system/searxng-wrapper.service
关键配置：
1
2
3
...
1
curl -sS
"http://127.0.0.1:8080/metrics?token=<METRICS_TOKEN>"
故障排除
Wrapper 连接被拒
1
2
...
8765
# 查看 systemd 日志
sudo journalctl -u searxng-wrapper.service -n
50
SearxNG 返回 403
确认 settings.yml 中
limiter: false
