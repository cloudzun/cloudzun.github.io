---
title: 'Hugo博客标准部署手册'
description: 'Hugo博客标准部署手册 概述 本文档详细介绍如何部署和维护一个使用Hugo构建的博客系统，包括本地安装、配置、通过Nginx Proxy Manager发布以及常见问题的解决方法。
'
...
6
# 使用snap安装（推荐）
sudo snap install hugo --channel
=
extended
# 或使用包管理器安装
sudo apt update
...
2
# 在博客目录下执行
hugo server --bind 0.0.0.0 --baseURL https://yourdomain.com --appendPort
=
false
--port
1313
...
4.2 避免常见陷阱
服务冲突
: 检查是否有系统服务自动启动Hugo（如hugo.service）
进程管理
: 使用
pkill -f hugo
彻底终止所有Hugo进程
...
症状
:
listen tcp 0.0.0.0:1313: bind: address already in use
解决方法
:
1
2
...
5.3 本地跳转问题
症状
: 访问域名时跳转到localhost:端口
解决方法
:
确保baseURL使用公网域名而非localhost
确保使用
--appendPort=false
参数
检查Nginx Proxy Manager的Forward配置
5.4 链接跳转到错误地址
症状
: 页面内的链接跳转到错误地址
解决方法
:
确保hugo.toml中的baseURL配置正确
重新启动Hugo服务器使配置生效
六、维护与更新
6.1 日常启动脚本
创建启动脚本
/home/user/start-hugo.sh
:
1
2
3
...
https://yourdomain.com
–appendPort=false –port 1313 &
echo “$(date): Hugo restarted” » /var/log/hugo-monitor.log
fi
七、经验教训总结
7.1 今日案例回顾
问题
: Hugo服务器启动后仍跳转到localhost地址
原因
: baseURL配置不正确，或有系统服务自动重启Hugo
解决方案
: 确保baseURL使用公网域名，并停用自动重启服务
7.2 最佳实践
操作前确认
: 在修改任何配置前，先确认服务器和域名的关系
渐进式修改
: 避免一次性进行多项修改
验证机制
: 每次修改后验证结果
